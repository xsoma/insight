...
void Skins::OnFrameStageNotify(ClientFrameStage_t stage)
{
		...
		return;
	{
		...
		const auto glove_config = &g_Options.m_skins.m_items[GLOVE_T_SIDE];
	...
	}
	{
		...
		for (int i = 0; weapons[i].IsValid(); i++)
		{
			...
			const auto active_conf = &g_Options.m_skins.m_items[is_knife(definition_index) ? WEAPON_KNIFE : definition_index];
