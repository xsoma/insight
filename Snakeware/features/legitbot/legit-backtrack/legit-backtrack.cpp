...
void LegitBacktrack::OnMove(CUserCmd *pCmd) {
	...
	g_LegitBot .settings = g_Options.legitbot_items[pWeapon->m_Item().m_iItemDefinitionIndex()];
	if (!g_LegitBot.settings.enabled) {
		data.clear();
		return;
	}
	...
	for (int i = 1; i <= g_GlobalVars->maxClients; ++i) {
		...
		if (!g_LegitBot.settings.deathmatch && player->m_iTeamNum() == g_LocalPlayer->m_iTeamNum()) {
			if (data.count(i) > 0)
				data.erase(i);
			continue;
		}
	...
	for (auto& node : data) {
		...
		for (auto& bd : cur_data) {
			...
			if (std::fabsf(deltaTime) > g_LegitBot.settings.backtrack_time)
				continue;

			Math::VectorAngles(bd.hitboxPos - localEyePos, angles);
			Math::FixAngles(angles);
			float fov = g_LegitBot.GetFovToPlayer(pCmd->viewangles, angles);
			if (best_fov > fov) {
				best_fov = fov;
