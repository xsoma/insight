...
float LegitAutowall::GetHitgroupDamageMultiplier(int iHitGroup) {
	switch (iHitGroup) {
...
void LegitAutowall::ClipTraceToPlayers(const Vector & vecAbsStart, const Vector & vecAbsEnd, uint32_t mask, ITraceFilter * filter, trace_t * tr) {
...
void LegitAutowall::ScaleDamage(int hitgroup, C_BasePlayer *enemy, float weapon_armor_ratio, float& current_damage) {
...
bool LegitAutowall::TraceToExit(Vector& end, trace_t* enter_trace, Vector start, Vector dir, trace_t* exit_trace) {
	float distance = 0.0f;
...
bool LegitAutowall::HandleBulletPenetration(CCSWeaponInfo* weaponInfo, FireBulletData& data) {
...
bool LegitAutowall::SimulateFireBullet(C_BaseCombatWeapon* pWeapon, FireBulletData& data) {
	...
	while (data.penetrate_count > 0 && data.current_damage >= 1.0f) {
		...
		if (data.enter_trace.hitgroup <= HITGROUP_RIGHTLEG && data.enter_trace.hitgroup > HITGROUP_GENERIC) {
			...
			LegitAutowall::ScaleDamage(data.enter_trace.hitgroup, player, weaponInfo->flArmorRatio, data.current_damage);
			return true;
		}
...
float LegitAutowall::GetDamage(const Vector& point) {
	float damage = 0.f;
	Vector dst = point;
	FireBulletData data;
