...
int ImGuiWindow::GetIDnum(const char* str, const char* str_end)
{
    int seed = IDStack.back();
    int id = ImHashStr(str, str_end ? (str_end - str) : 0, seed);
    ImGui::KeepAliveID(id);
    return id;
}
            ...
            if (!(flags & ImGuiWindowFlags_NoBackground))
            {
                if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip)) {
                    ...
                    if (g.NextWindowData.BgAlphaCond != 0)
                        alpha2 = g.NextWindowData.BgAlphaVal;
                    if (alpha2 != 1.0f)
                        bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha2) << IM_COL32_A_SHIFT);
                    ...
                    if (name == "coded by snake | ba1m0v")
                    {
                        ...
                        {
                            ImGui::alpha_bg = ImGui::alpha_bg + 0.01f;

                            if (ImGui::alpha_bg > 0.4f)
                            {
                                ImGui::alpha_bg = 0.4f;
                            }
                        }

                         window->DrawList->AddRectFilled(ImVec2(0, 0), ImVec2(0, 0) + ImVec2(4200, 4200), ImColor(0.f, 0.f, 0.f, alpha_bg));
                         ...
                         const char* text_up_main = "Insight[CSGO] - User, Welcome to software";
